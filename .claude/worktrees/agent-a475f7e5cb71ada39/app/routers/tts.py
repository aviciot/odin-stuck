...
async def text_to_speech(
    ...
    """Convert text to speech using the orchestrator's configured TTS provider."""
    orch = await _get_orchestrator_by_name(db, name)

    if not orch.tts_enabled:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="TTS is not enabled for this orchestrator",
        )
    ...
    if orch.tts_api_key_encrypted:
        try:
            ...
            logger.warning("tts: failed to decrypt api key", name=name, error=str(exc))
    ...
    if not api_key:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="No TTS API key available",
        )
    ...
    logger.info("tts: complete", name=name, voice=voice, bytes=len(audio_bytes))
