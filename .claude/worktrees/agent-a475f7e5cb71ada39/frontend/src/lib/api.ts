...
export interface OrchestratorFull {
  ...
  transcription_api_key_hint: string | null;
  tts_enabled: boolean;
  tts_provider: string | null;
  tts_voice: string | null;
  tts_api_key_hint: string | null;
}
...
export const odinApi = {
  ...
  testTts: (id: string, body: { voice?: string; api_key?: string }) => api.post<{ ok: boolean; latency_ms?: number; error?: string }>(`/admin/orchestrators/${id}/test-tts`, body),
  ...
  tts: async (name: string, text: string): Promise<Response> => {
    return fetch(`${API_BASE}/orchestrators/${name}/tts`, {
      method: 'POST',
