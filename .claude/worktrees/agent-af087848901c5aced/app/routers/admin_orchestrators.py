...
from app.utils.crypto import encrypt_value, key_hint
...
class OrchestratorCreate(BaseModel):
    ...
    transcription_provider: Optional[str] = None   # 'openai' | 'groq'
...
class OrchestratorUpdate(BaseModel):
    ...
    transcription_model: Optional[str] = None
    transcription_api_key: Optional[str] = None    # blank = keep existing
...
class OrchestratorOut(BaseModel):
    ...
    transcription_api_key_hint: Optional[str]       # masked
    enabled: bool
...
async def update_orchestrator(
    ...
    if body.transcription_api_key:                  # non-blank = update; blank = preserve
